Public Class ClPessoa
    ...
    Private dta_cadastro_ As String
    Public Property dta_cadastro() As String
        Get
    ...
    Private ind_preco_venda_ As String
    Public Property ind_preco_venda() As String
        Get
            Return ind_preco_venda_
        End Get
        Set(ByVal value As String)
            ind_preco_venda_ = value
        End Set
    End Property
    ...
    Public Sub CadastrarPessoa()
        ...
        sql = "Insert Into tab_pessoa(
                               ...
                               DES_EMAIL         ,
                               ind_preco_venda  )
                 Values (" & cod_pessoa & ",'" & nom_pessoa & "' , '" & nom_fantasia & "','" & num_cnpj_cpf & "','" & num_ie_rg & "'," & vbCrLf
        If dta_nascimento = "" Then
            sql = sql & "Null ,"
        Else
            sql = sql & " '" & CDate(dta_nascimento).ToShortDateString.ToString & "'," & vbCrLf
        End If
        sql = sql & " '" & des_logradouro & "','" & des_complemento & "'," & cod_cidade & ",'" & nom_bairro & "'
                         , '" & num_cep & "','" & num_fone_1 & "','" & num_fone_2 & "','" & num_fone_3 & "' 
                         , '" & des_observacao & "','" & CDate(dta_cadastro).ToShortDateString & "','" & ind_cliente & "','" & ind_fornecedor & "'
                         , '" & ind_funcionario & "','" & ind_ativo & "','" & des_email & "','" & ind_preco_venda & "')"
        con.Operar(sql)
    End Sub
    ...
    Public Sub AtualizarPessoa()
        sql = "Update tab_pessoa Set  NOM_PESSOA    ='" & nom_pessoa & "' ,
                                    ...
                                    NUM_IE_RG       ='" & num_ie_rg & "',"
        If dta_nascimento = "" Then
            sql = sql & "dta_nascimento  = null,"
        Else
            sql = sql & "dta_nascimento  ='" & dta_nascimento & "' ,"
        End If

        sql = sql & "  DES_LOGRADOURO  ='" & des_logradouro & "',
                                    ...
                                    IND_ATIVO       ='" & ind_ativo & "',
                                    DES_EMAIL       ='" & des_email & "',
                                    ind_preco_venda ='" & ind_preco_venda & "'
    ...
    Public Sub ExcluirPessoa()
        sql = " Delete from tab_pessoa Where cod_pessoa > 1  AND cod_pessoa =" & cod_pessoa & ""
        con.Operar(sql)
    End Sub
    ...
    Public Function ConsultarPessoa(busca As String)
        ...
        Return ds
    End Function
    Public Function ConsultarPessoaBusca(COD_PESSOA_OLD As Integer, NOM_PESSOA_OLD As String, NOM_FANTASIA_OLD As String, DTA_NASCIMENTO_OLD As String,
        ...
        End If
        If IsDate(DTA_NASCIMENTO_OLD) = False Then
            DTA_NASCIMENTO_OLD = ""
        End If
        ...
        sql = "select  a.COD_PESSOA   ,
              ...
              join tab_cidade d on (a.cod_cidade = d.cod_cidade)
               ...
               where a.nom_pessoa like ('%" & NOM_PESSOA_OLD & "%') 
                 ...
                 and a.nom_fantasia like ('%" & NOM_FANTASIA_OLD & "%') " & vbCrLf
        If DTA_NASCIMENTO_OLD <> "" Then
            sql = sql & "  and a.dta_nascimento = '" & CDate(DTA_NASCIMENTO_OLD).ToShortDateString & "'"
        End If
        sql = sql & " and ((a.num_cnpj_cpf = '" & NUM_CNPJ_CPF_OLD & "') or ('' = '" & NUM_CNPJ_CPF_OLD & "' ))
    ...
    Public Function ConsultarPessoaFuncionario(busca As String)
        ...
        Return ds
    End Function
    Public Function ConsultarPessoaFuncionarioAgenda(busca As String)
        sql = "Select a.cod_pessoa,
                      a.nom_pessoa 
               From tab_pessoa a
               join tab_pessoa_funcionario b on (a.cod_pessoa = b.cod_pessoa)
                where a.ind_funcionario = 'S' 
                  and a.ind_ativo = 'S'
                  and b.ind_agenda_servico = 'S'
                  and a.nom_pessoa like ('%" & busca & "%')
               order by a.nom_pessoa"
        ds = con.Listar(sql)
        Return ds
    End Function
    ...
    Public Function ConsultarPessoaCliente(busca As String)
        ...
        Return ds
    End Function
    Public Function ConsultarPessoaCodigo(busca As String)
        sql = "Select cod_pessoa,nom_pessoa From tab_pessoa
                where   ind_ativo = 'S'
                  and cod_pessoa = " & busca & "
               order by nom_pessoa"
        ds = con.Listar(sql)
        Return ds
    End Function
    ...
    Public Function UltimaPessoaCadastrado()
        sql = "Select coalesce( max(cod_pessoa),0)as ultimo From tab_pessoa"
